...
oc_command() {
    local command=$1
    if [ "$DEBUG" -eq 1 ]; then
        eval "$command" | tee -a $ERROR_LOG
    else
        eval "$command" >> $ERROR_LOG 2>&1
    fi
}
...
select_cluster() {
    ...
    if [[ -z "$TOKEN" ]] || ! oc_command "oc login --token='$TOKEN' --server='$SERVER_URL' --insecure-skip-tls-verify=true"; then
        handle_login "$SERVER_URL"
    fi
}

handle_login() {
    local SERVER_URL=$1
    echo_color "Введите ваш логин и пароль."
    ...
    if oc_command "oc login -u '$username' -p '$password' --server='$SERVER_URL' --insecure-skip-tls-verify=true"; then
        ...
        if [[ -n "$new_token" ]]; then
            echo_color "Токен успешно обновлен."
            clusters[index]="$SERVER_URL $new_token"
            printf "%s\\n" "${clusters[@]}" > "$CONFIG_FILE"
        else
            echo_color "Не удалось обновить токен."
            return 1
        fi
    else
        echo_color "Не удалось выполнить вход."
        return 1
    fi
}
